...
public class WorkIntentHandler implements RequestHandler {

    final IDataSource data;
    private static final Logger LOG = Logger.getLogger(WhoIsIntentHandler.class);

    public WorkIntentHandler(IDataSource dataSource) {
        data = dataSource;
    }
    ...
    public boolean canHandle(HandlerInput input) {
        ...
        return input.matches(intentName("WorkIntent"));
    }
    ...
    private String buildSpeechText(Person person) {
        ...
        return String.format("%s %s %s %s %s %s", PhrasesAndConstants.RELATIONS.getPronounString(relationship),
                PhrasesAndConstants.RELATIONS.getRelationShipString(relationship), person.getFirstName(),
                person.getLastName(), PhrasesAndConstants.WORK, person.getWork());
    }
