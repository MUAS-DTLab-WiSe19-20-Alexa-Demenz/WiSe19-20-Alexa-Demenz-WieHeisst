...
public class WhoIsIntentHandler implements RequestHandler {

    private static final Logger LOG = Logger.getLogger(WhoIsIntentHandler.class);

    final IDataSource data;

    public WhoIsIntentHandler(IDataSource dataSource) {
        data = dataSource;
    }
    ...
    public boolean canHandle(HandlerInput input) {
        ...
        return input.matches(intentName("WhoIsIntent"));
    }
    ...
    private String buildSpeechText(Person person) {
        String speechText = String.format("%s %s. %s ist ", PhrasesAndConstants.NAME_IS, person.getFirstName(),
                person.getFirstName());

        // get relationship
        PhrasesAndConstants.RELATIONS relationship = PhrasesAndConstants.RELATIONS.valueOf(person.getRelationToUser());

        // build relationshipString
        speechText += PhrasesAndConstants.RELATIONS.getPronounString(relationship) + " ";
        speechText += PhrasesAndConstants.RELATIONS.getRelationShipString(relationship) + ".";
